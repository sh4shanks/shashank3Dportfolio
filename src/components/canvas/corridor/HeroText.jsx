 ...
 * - SHASHANK in Rubik Scribble font (splits into letters during scroll)
...
const HeroText = ({ position = [0, 0.3, 0] }) => {
    ...
    // Letter positions for SHASHANK split effect
    const letters = useMemo(() => {
        ...
        return chars.map((char, i) => ({
            char,
            baseX: start + i * spacing,
            splitDir: (i - (chars.length - 1) / 2) * 0.65,
            delay: i * 0.04,
        }));
    }, []);
    ...
    const taglineWords = useMemo(() => [
        ...
        { text: 'student', baseX: -0.38, splitDir: -0.8, delay: 0 },
        { text: 'developer', baseX: 0.38, splitDir: 0.8, delay: 0 },
        { text: '/>', baseX: 0.85, splitDir: 1.5, delay: 0 },
    ], []);
    ...
    useFrame((state, delta) => {
        ...
        // Apply split to each letter of SHASHANK
    ...
    return (
        <group ref={groupRef} position={position} scale={[scale, scale, 1]}>
            {/* SHASHANK Letters - Rubik Scribble font with fade-in animation */}
