    ...
    return (
        <div className="sr-overlay" role="complementary" aria-label="Accessible navigation for 3D portfolio">
            ...
            <nav id="sr-main-nav" className="sr-only" aria-label="Portfolio rooms">
                <h1>Shashank Singh — Creative Developer Portfolio</h1>
                <h2>Portfolio Navigation</h2>

                {!hasEntered && (
                    <p>Welcome to Shashank Singh's interactive 3D portfolio. Click or press Enter on the doors to enter.</p>
                )}
                            ...
                            <div aria-label="Gallery room content">
                                ...
                                {projects && projects.length > 0 && (
                                    <ul>
                                        {projects.map((p, i) => (
                                            <li key={i}>
                                                <h4>{p.category ? `${p.category}: ${p.title}` : p.title}</h4>
                                                <p>{p.description}</p>
                                                {p.url && <a href={p.url}>Visit {p.category ? `${p.category}: ${p.title}` : p.title}</a>}
                                            </li>
                                        ))}
                                    </ul>
                                )}
