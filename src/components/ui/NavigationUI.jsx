...
const NavigationUI = () => {
    ...
    }, [isMenuOpen, isAudioMenuOpen, isAchievementsOpen, isThemeMenuOpen]);
        ...
        <div className="navigation-ui">
            ...
            {hasEntered && (
                <div className={`nav-controls ${isMenuOpen || isAudioMenuOpen ? 'menu-open' : ''} ${isUIHidden ? 'ui-hidden' : ''}`}>
                    ...
                    {/* Theme Toggle Button */}
                    <button
                        className={`nav-btn theme-btn ${isThemeMenuOpen ? 'open' : ''}`}
                        onClick={() => setIsThemeMenuOpen(!isThemeMenuOpen)}
                        aria-label={`Theme: ${theme}`}
                        aria-expanded={isThemeMenuOpen}
                        title={`Theme: ${theme}`}
                    >
                        <svg viewBox="0 0 24 24" className="icon-theme">
                            {resolvedTheme === 'dark' ? (
            ...
            {/* Theme Panel */}
            {hasEntered && (
                <div className={`theme-panel ${isThemeMenuOpen ? 'open' : ''}`} inert={!isThemeMenuOpen ? true : undefined}>
                    <div className="theme-card">
                        <div className="theme-header">
                            <h3>APPEARANCE</h3>
                            <button className="close-btn" onClick={() => setIsThemeMenuOpen(false)} aria-label="Close appearance settings">
